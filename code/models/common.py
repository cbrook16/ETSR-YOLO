...
class C3CA(nn.Module):
    ...
    def forward(self, x):
        return self._CoordAtt(self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), dim=1)))
...
class DetectMultiBackend(nn.Module):
    ...
    def forward(self, im, augment=False, visualize=False, val=False):
        ...
        # LOGGER.info(f'im.shape batch is {b}')
        if self.pt or self.jit:  # PyTorch
            ...
            # bb = self.bindings['images'].shape
            # LOGGER.info(f'bindings shape batch is {bb}')
